...
function TablaClientes() {
  const [clients, setClients] = useState(SAMPLE_DATA_TABLA);
  ...
  const handleAddClient = (form) => {
    const newClient = { ...form, id: nextId };
        ...
        <div onClick={() => setEditingCell(cellKey)} style={{ cursor: "pointer" }}>
          {isEditing ? (
            <select
              autoFocus
              value={client[field]}
              onChange={(e) => updateClient(client.id, field, e.target.value)}
      ...
      <div onClick={() => setEditingCell(cellKey)} style={{ cursor: "pointer", minHeight: 20 }}>
        {isEditing ? (
          <input
            ...
            onBlur={(e) => updateClient(client.id, field, type === "number" ? Number(e.target.value) : e.target.value)}
            onKeyDown={(e) => {
              if (e.key === "Enter") updateClient(client.id, field, type === "number" ? Number(e.target.value) : e.target.value);
            }}
            style={{
              padding: "4px 6px",
              fontSize: 13,
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: 13, color: COLORS.textLight, textAlign: "center", margin: "0 0 20px" }}>
          Cliente: <strong>{client.nombre}</strong> — {formatMoney(client.monto)}
        </p>
...
function ClientCard({ client, onMoveClick, onRejectClick }) {
  ...
  const isRejected = client.rechazado;
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: 15, fontWeight: 700, color: COLORS.text, margin: 0 }}>{client.nombre}</p>
...
function PortalEjecutivo() {
  const [clients, setClients] = useState(SAMPLE_CLIENTS_PORTAL);
  ...
  const ejecutivo = "Carlos Manuel Padilla Casanova";

  const filtered = useMemo(() => {
    if (filterEstatus === "todos") return clients;
    if (filterEstatus === "activos") return clients.filter((c) => !c.rechazado && c.estatus !== "Dispersión");
    if (filterEstatus === "cerrados") return clients.filter((c) => c.estatus === "Dispersión");
    if (filterEstatus === "rechazados") return clients.filter((c) => c.rechazado);
    return clients.filter((c) => c.estatus === filterEstatus && !c.rechazado);
  ...
  const handleMove = (id, newStatus) => {
    setClients((prev) => prev.map((c) => {
      if (c.id !== id) return c;
      const updated = { ...c, estatus: newStatus };
      if (newStatus === "Dispersión") updated.fecha_final = new Date().toISOString().split("T")[0];
      return updated;
    }));
    setMoveClient(null);
  };
      ...
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 6, minWidth: 600 }}>
            {STAGES_PORTAL.map((stage, idx) => {
              const count = clients.filter((c) => c.estatus === stage.key && !c.rechazado).length;
              return (
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: COLORS.bg }}>
      ...
      <main style={{
        ...
        {activeScreen === "clientes" && <TablaClientes />}
        ...
        {activeScreen === "portal" && <PortalEjecutivo />}
